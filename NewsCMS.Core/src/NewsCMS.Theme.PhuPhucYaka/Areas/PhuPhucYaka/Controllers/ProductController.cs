    ...
    public async Task<IActionResult> Index(string? cat, int p = 1, CancellationToken ct = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cat))
        {
            ...
            if (match is null)
            {
                ...
                return View(NewsCMS.Application.Common.PagedList<NewsCMS.Application.Catalog.Dtos.ProductListItemDto>.Empty(p, 9));
            }
            catId = match.Id;
        }

        var items = await _products.SearchPublishedAsync(catId, p, 9, ct);
        ViewBag.Categories = categories;
        ViewBag.SelectedCategorySlug = cat;
        return View(items);
    }
